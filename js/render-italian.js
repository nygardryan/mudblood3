...
// Roster keys and art variants aren't 1:1 — several types deliberately share a
// silhouette (the sapper carries the Fante's kit, the Moschettiere the MAB, the
// Arditi the Folgore's jump smock). Normalizing the live type into an ART KEY
// here keeps the dispatch below keyed on silhouettes instead of on roster
// bookkeeping, so adding a type that looks like an existing one costs one line.
const IT_ART = {
  iguast: 'irifle',
  ifante: 'irifle',
  imosch: 'imab',
...
function paintItalianSoldier(c, a) {
  const type = IT_ART[a.type] || a.type;
  const t = a.t;
  const gunLen = t.gun;
