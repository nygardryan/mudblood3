...
// M2 flamethrower wand — heat shield, grip, bell nozzle, fuel hose. The US half
// of the pair drawFlammenwerfer below is the German half of.
function drawM2Flamethrower(c, fx, fy, gunLen, lit) {
...
function drawBAR(c, fx, fy, gunLen, face) {
  const tipX = fx * gunLen, tipY = fy * gunLen;
  c.strokeStyle = '#26261e';
  c.lineWidth = 2.8;
  c.beginPath();
  ...
  for (const s of [-0.72, 0.72]) {
    ...
    c.lineTo(
      fx * (gunLen + 1.5) + Math.cos(face + s + 0.45) * 4,
      fy * (gunLen + 1.5) + Math.sin(face + s + 0.45) * 4,
    );
    c.stroke();
  }
  // flash hider
...
// M97 trench gun — long barrel, pump forend, wide choke, wooden stock. blastT is
// the firing timer: the forend slides back on recoil.
function drawTrenchGun(c, fx, fy, gunLen, blastT) {
  const tipX = fx * gunLen, tipY = fy * gunLen;
  const pumpT = blastT > 0 ? clamp(blastT / 0.12, 0, 1) : 0;
...
// The US flamer's back tanks — twin cylinders, cross straps, warning stripe.
function drawFlamerKit(c, fx, fy) {
  const tankX = -6.2;
...
// 12-gauge bandolier, shell loops and belt pouches — the trench gunner's kit.
function drawShotgunnerKit(c, fx, fy) {
  c.strokeStyle = '#8a7a48';
  c.lineWidth = 1.5;
  c.beginPath();
...
// The US mortarman's kit — ammo crate by the baseplate, chest shell bag and the
// range board under his arm. Takes the tube drawMortarTube returned, like
// drawEmortarKit does, so the crate lands beside the baseplate actually drawn.
function drawMortarKit(c, fx, fy, face, tube) {
  ...
  c.fillStyle = '#3a4034';
  c.beginPath(); c.ellipse(fy * 3.5, -fx * 3.5, 2.8, 3.4, face, 0, 7); c.fill();
  c.strokeStyle = '#2a2e24';
  c.lineWidth = 0.9;
  c.stroke();
  drawMortarRound(c, fy * 3.2, -fx * 3.2, 0.65, face + 0.5);
